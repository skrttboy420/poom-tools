...
import { analyzeNumClean, cleanNumber, STATUS_LABEL, type DecimalMode } from "@/lib/numclean/numclean";
...
const MODES: { id: DecimalMode; label: string; hint: string }[] = [
  { id: "us", label: "US (1,234.56)", hint: "จุลภาค = หลักพัน · จุด = ทศนิยม (deterministic)" },
  { id: "eu", label: "EU (1.234,56)", hint: "จุด = หลักพัน · จุลภาค = ทศนิยม" },
  { id: "auto", label: "เดาอัตโนมัติ", hint: "เดาตัวคั่นทศนิยมจากตำแหน่ง (ตัวท้ายสุด/3 หลักท้าย)" },
];
...
// นับช่องที่ "เป็นข้อความแต่ล้างเป็นตัวเลขได้" (ตัวที่ยังสกปรก) — คอลัมน์ที่ควรล้าง
function dirtyScore(data: Row[], c: number): number {
  let score = 0;
  for (const row of data.slice(0, 60)) {
    const v = c < row.length ? row[c] : null;
    if (v === null || v === undefined || v === "") continue;
    if (typeof v === "string") {
      const r = cleanNumber(v, { decimalMode: "us" });
      // ตัวที่ล้างได้แต่ String(number) ไม่ตรงกับเดิม = ยังสกปรก
      if (r.ok && r.value !== null && String(r.value) !== v.trim()) score++;
    }
  }
  return score;
}
// เดาคอลัมน์: หัวเข้าข่ายเงิน/ยอด/ราคา ก่อน ไม่งั้นคอลัมน์ที่มีตัวเลขสกปรกมากสุด
function guessCol(header: Row, data: Row[]): number {
  const keys = ["price", "ราคา", "amount", "ยอด", "เงิน", "total", "รวม", "มูลค่า", "cost", "ต้นทุน", "value", "จำนวนเงิน"];
  for (let i = 0; i < header.length; i++) {
    const h = String(header[i] ?? "").toLowerCase().trim();
    if (h !== "" && keys.some((k) => h.includes(k)) && dirtyScore(data, i) > 0) return i;
  }
  let best = -1;
  let bestScore = 0;
  for (let i = 0; i < header.length; i++) {
    const sc = dirtyScore(data, i);
    if (sc > bestScore) {
      bestScore = sc;
      best = i;
    }
...
export default function NumCleanPage() {
  const [state, setState] = useState<FileState | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);

  const [col, setCol] = useState(-1);
  const [decimalMode, setDecimalMode] = useState<DecimalMode>("us");
  const [pctFrac, setPctFrac] = useState(false);
  ...
  const result = useMemo(
    () =>
      analyzeNumClean(headerStr, data, {
        col,
        decimalMode,
        percentToFraction: pctFrac,
        round: doRound ? Math.max(0, Number(decimals) || 0) : null,
        mode: replace ? "replace" : "add",
        colName: colName.trim() === "" ? undefined : colName,
      }),
    [headerStr, data, col, decimalMode, pctFrac, doRound, decimals, replace, colName],
  );
  ...
  const dlCsv = () => {
    ...
    downloadText(changeExt(state.file.fileName, "csv", "-ล้างตัวเลข"), rowsToCsv([result.header, ...result.rows]), "text/csv");
  };
  const dlXlsx = () => {
    if (!state || result.error) return;
    downloadBlob(changeExt(state.file.fileName, "xlsx", "-ล้างตัวเลข"), rowsToXlsx([result.header, ...result.rows]), XLSX_MIME);
  };
  ...
  return (
    <main className="flex flex-1 flex-col">
      <header className="flex items-center justify-between border-b border-black/10 px-6 py-4 dark:border-white/10">
        <div>
          <h1 className="text-lg font-semibold">ล้างตัวเลขให้สะอาด 🧼</h1>
          <p className="text-xs text-neutral-500">
            ตัวเลขปนสัญลักษณ์เงิน/หลักพันแบบยุโรป/วงเล็บติดลบ/% (เช่น &quot;$1,234.56&quot; / &quot;1.234,56&quot; / &quot;(1,000)&quot;) →
            แปลงเป็น number จริง · ต่างจาก /clean (แค่ตัด comma US) · อ่านไม่ออก = เว้นว่าง ไม่เดามั่ว
          </p>
        </div>
          ...
          <div className="space-y-4 rounded-xl border border-black/10 p-4 dark:border-white/10">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1 text-xs text-neutral-600 dark:text-neutral-400">
                <span>คอลัมน์ตัวเลขที่จะล้าง</span>
                <select
                  value={col}
              ...
              <div>
                <span className="text-xs text-neutral-500">รูปแบบตัวคั่น:</span>
                <div className="mt-1 flex flex-wrap gap-2">
                  {MODES.map((m) => (
                    <button
                      key={m.id}
                      onClick={() => setDecimalMode(m.id)}
                      title={m.hint}
                ...
                <p className="mt-1 text-[11px] text-neutral-400">{MODES.find((m) => m.id === decimalMode)?.hint}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-end gap-6 border-t border-black/5 pt-3 dark:border-white/5">
              <label className="flex cursor-pointer items-center gap-1.5 text-xs text-neutral-600 dark:text-neutral-400">
                <input type="checkbox" checked={pctFrac} onChange={(e) => setPctFrac(e.target.checked)} />
                &quot;50%&quot; → 0.5 (หารร้อย)
              </label>
              <label className="flex cursor-pointer items-center gap-1.5 text-xs text-neutral-600 dark:text-neutral-400">
              ...
              {result.error ? (
                ...
                <>
                  <span className="rounded-full bg-emerald-100 px-3 py-1 font-medium text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">
                    ล้างแล้ว <span className="tabular-nums">{result.cleanedRows}</span>
                  </span>
                  {result.alreadyRows > 0 && (
                    <span className="rounded-full bg-sky-100 px-3 py-1 font-medium text-sky-800 dark:bg-sky-900/40 dark:text-sky-300">
                      เป็นตัวเลขอยู่แล้ว <span className="tabular-nums">{result.alreadyRows}</span>
                    </span>
                  )}
                  {result.unparseableRows > 0 && (
                    <span className="rounded-full bg-amber-100 px-3 py-1 font-medium text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">
                      อ่านไม่ออก (เว้นว่าง) <span className="tabular-nums">{result.unparseableRows}</span>
                    </span>
                  )}
                  {result.blankRows > 0 && (
