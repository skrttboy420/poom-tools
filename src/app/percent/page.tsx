...
import { analyzeColumn, METRICS, type Metric } from "@/lib/percent/percent";
...
// เดาคอลัมน์ตัวเลข: หาคอลัมน์ที่ค่าเป็นตัวเลขเยอะสุด (ข้ามคอลัมน์แรกที่มักเป็น key/tracking)
function guessNumericCol(header: Row, data: Row[]): number {
  let best = -1;
  let bestScore = 0;
...
export default function PercentPage() {
  ...
  const [metrics, setMetrics] = useState<Metric[]>(["share", "running"]);
  ...
  const effectiveCol = colTouched ? col : guessNumericCol(header, data);

  const result = useMemo(
    () =>
      analyzeColumn(headerStr, data, {
        col: effectiveCol,
        metrics,
        rankDescending: rankDesc,
        round: roundOn ? Math.max(0, Number(roundPlaces) || 0) : null,
      }),
    [headerStr, data, effectiveCol, metrics, rankDesc, roundOn, roundPlaces],
  );
  ...
  const dlCsv = () => {
    ...
    downloadText(
      changeExt(state.file.fileName, "csv", "-สัดส่วน"),
  ...
  const dlXlsx = () => {
    if (!state || result.error) return;
    downloadBlob(
      changeExt(state.file.fileName, "xlsx", "-สัดส่วน"),
  ...
  const firstNew = result.firstNewColIndex;
  ...
  return (
    <main className="flex flex-1 flex-col">
      <header className="flex items-center justify-between border-b border-black/10 px-6 py-4 dark:border-white/10">
        <div>
          <h1 className="text-lg font-semibold">% สัดส่วน &amp; ยอดสะสม 📈</h1>
          <p className="text-xs text-neutral-500">
            เติมคอลัมน์วิเคราะห์ต่อแถว — % สัดส่วนของยอดรวม · ยอดสะสม · % สะสม · อันดับ ·
      ...
      <section className="flex-1 space-y-6 p-6">
        ...
        {state && (
          <div className="space-y-3 rounded-xl border border-black/10 p-4 dark:border-white/10">
            <h2 className="text-sm font-semibold">เลือกคอลัมน์ &amp; ค่าที่จะวิเคราะห์</h2>

            <label className="flex flex-wrap items-center gap-2 text-sm">
              <span className="text-xs text-neutral-500">คอลัมน์ตัวเลข:</span>
              <select
                value={effectiveCol}
                onChange={(e) => {
                  setColTouched(true);
              ...
              {result.error ? (
                ...
                <>
                  <span className="rounded-full bg-emerald-100 px-3 py-1 font-medium text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">
                    ยอดรวม <span className="tabular-nums">{result.total.toLocaleString()}</span>
                  </span>
                  ...
                  <span className="text-neutral-400">เติม {result.addedCols} คอลัมน์</span>
                </>
              )}
                    ...
                    {result.header.map((h, i) => (
                      <th
                        ...
                        className={`px-2 py-1.5 whitespace-nowrap ${
                          isNewCol(i) ? "text-emerald-700 dark:text-emerald-300" : ""
                        }`}
                      >
                        {h === "" ? columnLetter(i) : h}
                        {isNewCol(i) && " 📈"}
