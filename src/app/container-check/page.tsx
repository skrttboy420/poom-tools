...
import { checkContainers } from "@/lib/containercheck/containercheck";
...
// เดาคอลัมน์เลขตู้: จับชื่อหัวก่อน (ตู้/container/cntr) ไม่งั้นสแกนหาคอลัมน์ที่ค่าดูเป็นเลขตู้มากสุด
const HEADER_KEYS = ["เลขตู้", "ตู้", "container", "cntr", "cont no", "container no", "คอนเทนเนอร์"];
const CONTAINER_RE = /^\s*[A-Za-z]{4}[\s-]?\d{6}[\s-]?\d\s*$/;
...
function guessColumn(header: Row, data: Row[]): number {
  ...
  // 2) สแกนค่า: คอลัมน์ไหนมีค่าที่ match รูปเลขตู้มากสุด
  ...
  for (let c = 0; c < width; c++) {
    ...
    for (const r of sample) {
      const v = r[c];
      if (typeof v === "string" && CONTAINER_RE.test(v)) hits++;
    }
    if (hits > bestHits) {
      bestHits = hits;
      best = c;
...
export default function ContainerCheckPage() {
  ...
  const result = useMemo(() => checkContainers(headerStr, data, col), [headerStr, data, col]);
  ...
  const dlCsv = () => {
    ...
    downloadText(changeExt(state.file.fileName, "csv", "-ตรวจตู้"), rowsToCsv([result.header, ...result.rows]), "text/csv");
  };
  const dlXlsx = () => {
    if (!state || result.error) return;
    downloadBlob(changeExt(state.file.fileName, "xlsx", "-ตรวจตู้"), rowsToXlsx([result.header, ...result.rows]), XLSX_MIME);
  };
  ...
  const hlCol = result.newColIndex; // คอลัมน์ผลตรวจที่เพิ่ม (ไฮไลต์)

  return (
    <main className="flex flex-1 flex-col">
      <header className="flex items-center justify-between border-b border-black/10 px-6 py-4 dark:border-white/10">
        <div>
          <h1 className="text-lg font-semibold">ตรวจเลขตู้คอนเทนเนอร์ 📦</h1>
          <p className="text-xs text-neutral-500">
            ตรวจเช็คดิจิต (check digit) ตามมาตรฐาน ISO 6346 — จับเลขตู้พิมพ์ผิดก่อนเอาเข้า Pacred · ไม่แก้เลขเดิม แค่เพิ่มคอลัมน์ &ldquo;ผลตรวจ&rdquo;
          </p>
        </div>
      ...
      <section className="flex-1 space-y-6 p-6">
        ...
        <div className="rounded-xl border border-black/10 p-4 dark:border-white/10">
          <div className="mb-2 flex items-center justify-between">
            <h2 className="text-sm font-semibold">ไฟล์ที่มีเลขตู้</h2>
            {state && (
              <span
        ...
        {state && (
          <div className="space-y-3 rounded-xl border border-black/10 p-4 dark:border-white/10">
            ...
            <p className="text-xs text-neutral-400">
              รูปแบบเลขตู้ ISO 6346: ตัวอักษร 4 ตัว + ตัวเลข 7 ตัว (เช่น CSQU3054383) · ตัวสุดท้าย = เช็คดิจิต ·
              เว้นวรรค/ขีดในเลขจะถูกตัดให้อัตโนมัติ
            </p>
          </div>
        )}
            ...
            {!result.error && result.findings.length > 0 && (
              <div className="border-b border-black/10 bg-rose-50/40 px-3 py-2 dark:border-white/10 dark:bg-rose-950/10">
                <p className="mb-1.5 text-xs font-medium text-rose-700 dark:text-rose-300">
                  เลขตู้ที่มีปัญหา ({result.findings.length}
