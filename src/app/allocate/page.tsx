...
import { analyzeAllocate } from "@/lib/allocate/allocate";
...
// เดาคอลัมน์ฐานปันส่วน: หัวตารางเข้าข่าย น้ำหนัก/CBM/จำนวน ก่อน ไม่งั้นคอลัมน์ตัวเลขที่ค่ามากสุด
function guessBasisCol(header: Row, data: Row[]): number {
  const keys = ["น้ำหนัก", "weight", "kg", "cbm", "คิว", "ปริมาตร", "volume", "จำนวน", "qty", "กล่อง"];
...
export default function AllocatePage() {
  ...
  const [basisCol, setBasisCol] = useState(-1);
  ...
  const reguess = useCallback((rows: Row[], sel: SideSelection) => {
    ...
    setBasisCol(guessBasisCol(hdr, dat));
  }, []);

  const totalNum = Number(total.replace(/,/g, "").trim());
  const result = useMemo(
    () =>
      analyzeAllocate(headerStr, data, {
        basisCol,
        total: total.trim() === "" ? NaN : totalNum,
        round: roundOn ? Math.max(0, Number(roundPlaces) || 0) : 2,
        amountName: amountName.trim() === "" ? undefined : amountName,
        showShare,
      }),
    [headerStr, data, basisCol, total, totalNum, roundOn, roundPlaces, amountName, showShare],
  );
  ...
  const dlCsv = () => {
    ...
    downloadText(changeExt(state.file.fileName, "csv", "-ปันส่วน"), rowsToCsv([result.header, ...result.rows]), "text/csv");
  };
  const dlXlsx = () => {
    if (!state || result.error) return;
    downloadBlob(changeExt(state.file.fileName, "xlsx", "-ปันส่วน"), rowsToXlsx([result.header, ...result.rows]), XLSX_MIME);
  };
  ...
  return (
    <main className="flex flex-1 flex-col">
      <header className="flex items-center justify-between border-b border-black/10 px-6 py-4 dark:border-white/10">
        <div>
          <h1 className="text-lg font-semibold">ปันส่วนต้นทุนตามสัดส่วน ⚖️</h1>
          <p className="text-xs text-neutral-500">
            มีค่าขนส่ง/ต้นทุนรวม 1 ก้อน → ปันลงแต่ละแถวตาม &quot;ฐาน&quot; (น้ำหนัก/CBM/จำนวน) · ผลรวมที่ปัน = ยอดรวมเป๊ะเสมอ
            (เศษปัดไม่ทำเงินหาย/เกิน) · ฐานไม่ใช่ตัวเลข/ติดลบ = เว้นว่าง ไม่เดามั่ว · ไม่แตะข้อมูลเดิม
          </p>
        </div>
          ...
          <div className="space-y-4 rounded-xl border border-black/10 p-4 dark:border-white/10">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1 text-xs text-neutral-600 dark:text-neutral-400">
                <span>คอลัมน์ฐานปันส่วน (ตัวเลข เช่น น้ำหนัก/CBM/จำนวน)</span>
                <select
                  value={basisCol}
                  onChange={(e) => setBasisCol(Number(e.target.value))}
              ...
              <label className="flex flex-col gap-1 text-xs text-neutral-600 dark:text-neutral-400">
                <span>ยอดรวมที่จะปันส่วน (บาท ฯลฯ)</span>
                <input
                  type="text"
                  inputMode="decimal"
                  value={total}
                  ...
                  className="rounded border border-black/15 bg-transparent px-2 py-1 tabular-nums dark:border-white/15 dark:bg-neutral-900"
                />
              </label>
            </div>
            ...
            <div className="flex flex-wrap items-end gap-6 border-t border-black/5 pt-3 dark:border-white/5">
              <label className="flex flex-col gap-1 text-xs text-neutral-600 dark:text-neutral-400">
                <span>ชื่อคอลัมน์ยอดปันส่วน</span>
                <input
                  type="text"
                  value={amountName}
                  onChange={(e) => setAmountName(e.target.value)}
                  placeholder="ยอดปันส่วน"
              ...
              {result.error ? (
                ...
                <>
                  <span className="rounded-full bg-emerald-100 px-3 py-1 font-medium text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">
                    ปันได้ <span className="tabular-nums">{result.allocatedRows}</span> แถว
                  </span>
                  {result.skippedRows > 0 && (
                    <span className="rounded-full bg-amber-100 px-3 py-1 font-medium text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">
                      ฐานไม่ใช่ตัวเลข (เว้นว่าง) <span className="tabular-nums">{result.skippedRows}</span>
                    </span>
                  )}
                  <span className="text-neutral-400">
                    ผลรวมฐาน <span className="font-medium text-neutral-600 dark:text-neutral-300 tabular-nums">{result.basisTotal}</span>
                  </span>
                  <span className="rounded-full bg-sky-100 px-3 py-1 font-medium text-sky-800 dark:bg-sky-900/40 dark:text-sky-300">
                    รวมที่ปันส่วน <span className="tabular-nums">{result.allocatedTotal}</span>
                  </span>
                </>
              )}
