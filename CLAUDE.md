...
- 2026-07-11 — **เครื่องมือที่ 74 พร้อมใช้: แทรกแถวยอดย่อย (Subtotal) 📑** (`/subtotal`) — หมวด excel · เก็บ "ทุกแถวรายละเอียด" ครบ แล้ว **"แทรก" แถวยอดรวมต่อกลุ่ม** คั่นแต่ละกลุ่ม + แถวรวมทั้งหมดท้ายสุด (เลียนแบบ Excel Data ▸ Subtotal) · use-case จริง: packing list ต่อตู้ → เก็บรายละเอียดทุกกล่องไว้ครบ แล้วแทรกแถวรวม kg/CBM/กล่อง ต่อตู้ → ได้รายงานปริ้นต์ได้เลยในไฟล์เดียว
  · **ต่างจากญาติ:** /group /rollup (ยุบทั้งกลุ่มเหลือ 1 แถว — รายละเอียดหาย) · /group-share (เติมคอลัมน์ % ราย "แถว") · /pivot (ตารางไขว้) — อันนี้ = เก็บ "ทุกแถวเดิม" ครบ แล้วแค่ "แทรก" แถวยอดย่อย/กลุ่ม + แถวรวมท้าย (ไม่ยุบ ไม่ทำข้อมูลหาย)
  · engine `src\lib\subtotal\subtotal.ts` (pure): `analyzeSubtotal(header, allRows, opts)` → `SubtotalResult` (rows แบน = detail + subtotal ต่อกลุ่ม + grand) + `subtotalToCsv`
    - จัดกลุ่มหลายคอลัมน์ (composite key, label join " · ") · `regroup` (default true) จัดแถวกลุ่มเดียวกันติดกันก่อน (Map เสถียร) vs false ตัดกลุ่มเมื่อคีย์ "ติดกัน" เปลี่ยน (แบบ Excel ต้อง sort ก่อน) · `grandTotal` (default true) · `labelCol` (default groupCols แรก, ถ้าชนคอลัมน์ยอด → เลื่อนไป groupCol แรกที่ไม่ใช่ยอด) · `round?` (null = cleanFloat ล้าง noise)
    - **ปรัชญาไม่เดามั่ว/ไม่ทำข้อมูลหาย:** ทุกแถวรายละเอียดอยู่ครบ · ช่องไม่ใช่ตัวเลข = ไม่นับเข้ายอด · **คอลัมน์ที่ไม่มีเลขเลยในกลุ่ม → เว้นว่าง (ไม่กุ 0)** · คีย์ว่างคงเป็นกลุ่ม "(ว่าง)" · `subtotalRowIndexes`/`grandTotalIndex` บอกตำแหน่งแถวยอด · invariant: output.length = dataRows + groups + (grandTotal?1:0)
    - error: no-data / ไม่เลือก groupCol ("เลือกคอลัมน์ที่จะจัดกลุ่มอย่างน้อย 1 คอลัมน์") / ไม่เลือก sumCol ("เลือกคอลัมน์ตัวเลขที่จะรวมยอดอย่างน้อย 1 คอลัมน์")
  · UI `src\app\subtotal\page.tsx` (client): reuse parse/detect/columns/FileDropzone → อัปโหลด → auto-guess (key=container/ตู้/forwarder, sum=kg/cbm/box) → chips คอลัมน์จัดกลุ่ม (sky) + คอลัมน์รวมยอด (emerald) + toggle regroup/grandTotal/ปัดทศนิยม → ตารางผล (แถว subtotal ไฮไลต์ sky, แถว grand ไฮไลต์ emerald, cap 400 แถว) + ดาวน์โหลด CSV/Excel
  · verify 2 ชั้น: (1) **Node test 52/52 ผ่าน**: subtotal ต่อกลุ่ม + grand, composite key, regroup true/false, blank-skip (คอลัมน์ไม่มีเลข→เว้นว่าง), labelCol fallback เมื่อชนคอลัมน์ยอด, round, subtotalRowIndexes/grandTotalIndex ถูก, emptyKeyGroups, invariant length, error (no-data/no-groupCol/no-sumCol), input ไม่ mutate
    (2) **Chrome UI จริง** (CSV `container,tracking,kg,cbm,box` TU-A×2 + TU-B×3 มีแถว kg/cbm ว่าง): จัดกลุ่ม container, sum kg/cbm/box → **TU-A รวม 30/0.3/5** · **TU-B รวม 20/0.2/4** (KY005 kg/cbm ว่าง = ไม่นับ, box ยังนับ) · **รวมทั้งหมด 50/0.5/9** · แถว subtotal ไฮไลต์ sky, grand ไฮไลต์ emerald, detail อยู่ครบ · CSV 9 บรรทัด (หัว+5 detail+2 subtotal+1 grand) type csv · Excel PK magic 17201B spreadsheetml · **console สะอาด**
- 2026-07-13 — **เครื่องมือที่ 75 พร้อมใช้: เติมค่าให้ช่องว่าง 🪣** (`/fill-blank`) — หมวด excel · เติม "เฉพาะช่องว่าง" ในคอลัมน์ที่เลือก ด้วยค่าคงที่ (เช่น 0, "รอตรวจ", N/A) หรือค่าที่พบบ่อยสุดในคอลัมน์ (mode) · use-case จริง: ก่อน export เข้า Pacred อยากให้ช่องว่างมีค่า default — น้ำหนัก/CBM ที่เว้นว่าง → 0, สถานะเว้นว่าง → "รอตรวจ", forwarder เว้นว่าง → เติมค่าที่พบบ่อยสุดในคอลัมน์นั้น
  · **ต่างจากญาติ:** /fill (เติมค่าลง-ขึ้นจาก "ค่าล่าสุด" ในคอลัมน์เดียว แนวตั้ง) · /coalesce (เลือกค่าแรกไม่ว่าง "ข้ามคอลัมน์" แนวนอน) · /map (แทนค่าตามพจนานุกรม — ค่าที่มีอยู่) · /replace (ค้นหา-แทนที่ — ข้ามช่องว่าง) — อันนี้ = เติม "เฉพาะช่องว่าง" ด้วยค่าคงที่หรือค่าที่พบบ่อยสุด
  · engine `src\lib\fillblank\fillblank.ts` (pure): `fillBlank(header, allRows, {cols, mode, value, trimBlank, coerceNumber})` → `FillBlankResult` (header/rows/cols/perCol/filledCells/blankCells/rowsAffected/inputRows/dataRows/droppedBlankRows/error?)
    - 2 โหมด (`FillBlankMode`): **constant** (default, เติมด้วย value คงที่ — `coerceConstant` แปลงรูปตัวเลขล้วนเป็น number **แต่คงเลข 0 นำหน้า "007" เป็น string** ตามปรัชญา no-data-loss) · **mode** (เติมด้วยค่าที่พบบ่อยสุดในคอลัมน์นั้น — `columnMode` นับเฉพาะช่องไม่ว่าง, tie แตกด้วยลำดับพบครั้งแรก, ไม่มีค่าไม่ว่างเลย → เติมไม่ได้ (เว้นว่าง ไม่กุค่า))
    - **ปรัชญา = เติมเฉพาะช่องว่างเท่านั้น — ไม่ทับค่าที่มีอยู่, ไม่ทำแถวหาย/ไม่เพิ่มแถว:** ตัดแถวว่างทั้งแถวก่อน (isDataRow — กันเติมแถวผี) · `trimBlank` (default on: ช่องเว้นวรรคล้วน = ว่าง) · perCol stat (filled/blank/fillValue ต่อคอลัมน์) · rows.length = dataRows, input ไม่ mutate
    - error: no-data / ไม่เลือกคอลัมน์ ("เลือกคอลัมน์ที่จะเติมช่องว่างอย่างน้อย 1 คอลัมน์") / constant mode value ว่าง ("ใส่ค่าที่จะเติมในช่องว่าง")
  · UI `src\app\fill-blank\page.tsx` (client): reuse parse/detect/columns/FileDropzone → อัปโหลด → เลือก header → chips คอลัมน์ (emerald) + toggle โหมด (ค่าคงที่ ✏️ / ค่าที่พบบ่อยสุด 📊) + ช่องค่า (constant, ปุ่มลัด "0") + toggle แปลงตัวเลข (constant) / มองช่องว่างล้วนเป็นว่าง → chips (เติมกี่ช่อง/ช่องว่างที่พบ/แถวถูกแตะ/ตัดแถวว่าง) + ตาราง (ช่องที่เติมไฮไลต์เขียว) + ดาวน์โหลด CSV/Excel
  · verify: (1) **Node test 25/25 ผ่าน** (anchor column กันแถวถูกตัด): constant เติม 0/007 คง string, mode พบบ่อยสุด + tie ลำดับแรก, เติมเฉพาะช่องว่าง ไม่ทับค่าเดิม, ตัดแถวว่างทั้งแถว (droppedBlankRows), trimBlank on/off, perCol stat, mode ไม่มีค่าไม่ว่าง→เติมไม่ได้, error (no-data/ไม่เลือกคอลัมน์/value ว่าง), input ไม่ mutate · (2) gate เขียว (TSC_OK + lint สะอาด) · Chrome UI end-to-end รอภูมล็อกอินใหม่ที่ http://127.0.0.1:3000/login (session หมดอายุ)
- 2026-07-15 — **รื้อเป็น SaaS: sidebar ซ้าย + Dashboard หน้าแรก (Step A+B) เสร็จ + verify แล้ว** (commit 7803560)
  · **แนวคิด:** เปลี่ยนจาก single-page tool hub → เปลือกแอปแบบ SaaS (sidebar ค้างซ้าย + พื้นที่เนื้อหาขวา) + หน้าแรกเป็น Dashboard hi-tech (ทางลัด + สถิติการใช้งาน)
  · **Step A — shell:** `src\components\AppShell.tsx` (client) ห่อทุกหน้า — desktop: sidebar sticky `w-64` · mobile: ปุ่ม ☰ เปิด drawer ทับ · ซ่อน shell ในหน้า `/login` (คืน children เปล่า) · ปิด drawer ตอนเปลี่ยนหน้าแบบ set-state-during-render (เลี่ยงกฎ set-state-in-effect ของ React 19) · เรียก `useTrackVisit()` ก่อน early-return (rules-of-hooks)
    - `src\components\Sidebar.tsx`: brand poom-tools + ช่องค้นหา + ⭐ รายการโปรด + หมวดหมู่พับได้ (10 หมวด) + ปุ่มออกจากระบบ
    - `src\app\layout.tsx`: ห่อ `<AppShell>{children}</AppShell>` (คง THEME_INIT + ThemeToggle เดิม)
  · **Step B — Dashboard:** `src\components\Dashboard.tsx` (client, ~294 บรรทัด) หน้าแรกใหม่แทน ToolHub — hero gradient (emerald/sky/violet) + คำทักไทย "สวัสดี {name} 👋 วันนี้อยากทำอะไร?" + ช่องค้นหา autoFocus · โหมดค้นหา → กริดผลลัพธ์ · โหมดปกติ → 5 StatCards (พร้อมใช้/รายการโปรด/เคยใช้แล้ว/หมวดหมู่/เข้าใช้รวม) + ทางลัด (⭐ โปรด · 🔥 ใช้บ่อย · 🕘 ใช้ล่าสุด) + กริดพร้อมใช้ตามหมวด
    - `src\lib\tools\useUsage.ts` (`poom-tools:usage`): ติดตามการเข้าใช้เครื่องมือ (count/last) ผ่าน `useSyncExternalStore` + `useTrackVisit()` (บันทึกเมื่อ path ตรง `TOOLS[].href`) → feed ทางลัด 🔥/🕘 + StatCards
    - `src\lib\tools\useFavs.ts`: store รายการโปรดใช้ร่วม (Dashboard + Sidebar) · StatCard `ACCENTS` ใช้ class เต็ม (กฎ Tailwind v4 ห้ามประกอบ string สี)
  · verify Chrome จริง (`http://127.0.0.1:3000/`): หน้าแรก render ครบ — hero "สวัสดี pasit 👋", "มี 79 เครื่องมือพร้อมใช้", 5 StatCards (79/1/2/10/2) · เข้า /cbm + /convert → 🔥 ใช้บ่อย + 🕘 ใช้ล่าสุด (เรียง recency ถูก, convert ล่าสุดขึ้นก่อน) + StatCard เคยใช้แล้ว 0→2 · ⭐ โปรด = Gap Finder · sidebar อยู่ครบ · gate เขียว (eslint สะอาด) · console สะอาด (เหลือแต่ MetaMask warning ของ extension)
